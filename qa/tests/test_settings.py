...
class TestSettings:
    ...
    def test_update_user_info_successfully(self, driver, user_data):
        ...
        try:
            ...
            self.logger.info("✅ 기대 결과 6: 업데이트 후 Header - [마이프로필] 영역에 {이미지 링크}와 {사용자명}이 노출됨")
    ...
    def test_update_user_password_successfully(self,driver,user_data):
        ...
        try:
            ...
            assert settingsPage.get_password_input_value() == user_data["password"]
            ...
            assert header.is_my_profile_link_appear()
            self.logger.info("✅ 기대 결과 3: 변경된 비밀번호로 로그인 됨")
