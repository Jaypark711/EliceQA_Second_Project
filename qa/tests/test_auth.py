...
class TestAuthentication:
    ...
    def test_fail_signup(self, driver, username, email, password, expected_result, description):
        self.logger.info(f"▶️ {description}(으)로 회원가입 테스트 시작")
        header = Header(driver)
        ...
        try:
            ...
            assert signUpPage.get_username_input_value() == username
            self.logger.info("✅ 기대 결과 2: 사용자명이 Username 입력 필드에 정상 반영됨")

            signUpPage.send_email_input(email)
            assert signUpPage.get_email_input_value() == email
            self.logger.info("✅ 기대 결과 3: 이메일이 Email 입력 필드에 정상 반영됨")

            signUpPage.send_password_input(password)
            assert signUpPage.get_password_input_value() == password
            self.logger.info("✅ 기대 결과 4: 비밀번호가 Password 입력 필드에 정상 반영됨") 

            signUpPage.click_sign_up_btn()
            if isinstance(expected_result, str):
                assert expected_result in signUpPage.get_error_message_text()
            else:
                assert set(signUpPage.get_error_messages_text()) == set(expected_result)
            self.logger.info(f"✅ 기대 결과 5: {description}(으)로 회원가입 테스트 시 {expected_result} 경고창 출력 확인")
            self.logger.info(f"🎉 {description}(으)로 회원가입 테스트 완료")
    ...
    def test_successful_signin(self, driver):
        ...
        try:
            ...
            assert signInPage.get_email_input_value() == VALID_USER["email"]
            self.logger.info("✅ 기대 결과 2: 이메일이 Email 입력 필드에 정상 반영됨")

            signInPage.send_password_input(VALID_USER["password"])
            assert signInPage.get_password_input_value() == VALID_USER["password"]
    ...
    def test_redirection(self, driver):
        ...
        try:
            # 테스트 시나리오 시작
            header.click_sign_in_link()
            assert header.is_go_to_sign_in_page()
